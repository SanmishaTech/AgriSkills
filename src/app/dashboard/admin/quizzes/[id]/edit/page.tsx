...
export default function EditQuizPage() {
  ...
  const fetchQuizDetail = async (token: string) => {
    try {
      ...
      const formQuestions: Question[] = quiz.questions.map((q: any) => ({
        id: q.id,
        text: q.text,
        type: q.type as 'multiple_choice' | 'true_false' | 'fill_in_blank',
        points: q.points,
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-xl p-8 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <button
                  onClick={() => router.push(`/dashboard/admin/quizzes/${quizId}`)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50">
                  {quiz?.chapter.title} ({quiz?.chapter.course.title})
                </div>
                <p className="text-xs text-gray-500 mt-1">Cannot change chapter after creation</p>
              </div>
            </div>
