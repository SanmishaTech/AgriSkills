  ...
  const handleEditCourse = async () => {
    ...
    try {
      ...
      setTopic(prev => prev ? {
        ...prev,
        subtopics: (prev.subtopics || []).map(subtopic =>
          subtopic.courses.some(course => course.id === selectedCourse.id)
            ? { ...subtopic, courses: subtopic.courses.map(course => 
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      ...
      setTopic(prev => prev ? {
        ...
        subtopics: (prev.subtopics || []).map(subtopic =>
          subtopic.courses.some(course => course.id === selectedCourse.id)
            ? { ...subtopic, courses: subtopic.courses.filter(course => course.id !== selectedCourse.id) }
            : subtopic
        )
      } : null);
                          ...
                          {subtopic.courses.map((course) => (
                            <div key={course.id} className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                              ...
                              <div className="bg-white rounded-lg p-3 mb-4">
                                <h5 className="text-xs font-medium text-gray-500 mb-1">CONTENT PREVIEW</h5>
